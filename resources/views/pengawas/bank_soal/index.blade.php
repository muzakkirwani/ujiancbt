...
<main class="ml-0 md:ml-72 p-4 md:p-10 min-h-screen">
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-slate-800">Bank Soal Guru</h1>
            <p class="text-slate-500 font-medium mt-1">Kelola bank soal mandiri Anda untuk pelaksanaan ujian.</p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <div class="bg-white border border-slate-100 rounded-2xl p-5 hover:border-indigo-200 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                        ...
                        <div class="flex justify-end gap-2 pt-4 border-t border-slate-100">
                            <a href="{{ route('pengawas.bank_soal.soal.index', $b->id) }}" 
                            ...
                            <form action="{{ route('pengawas.bank_soal.destroy', $b->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus Bank Soal ini? Semua soal di dalamnya akan terhapus.')">
                                @csrf
                                @method('DELETE')
...
<div id="modal-add" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm hidden overflow-y-auto">
    <div class="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl p-6 md:p-10 transform transition-all my-8">
        ...
        <form action="{{ route('pengawas.bank_soal.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>
...
function editBankSoal(data) {
    ...
    let url = "{{ route('pengawas.bank_soal.update', ':id') }}".replace(':id', data.id);
