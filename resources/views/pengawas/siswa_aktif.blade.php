                        ...
                        <td class="px-4 py-2.5 text-right">
                            <div class="flex justify-end gap-2">
                                @if ($isActive || $isDisconnected)
                                    <form action="{{ route('pengawas.siswa_aktif.selesai', $aktif->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin secara paksa menyelesaikan ujian untuk siswa ini? Seluruh hasil jawaban akan dikunci dan dinilai otomatis.')">
                                        @csrf
                                ...
                                <form action="{{ route('pengawas.siswa_aktif.reset', $aktif->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin me-reset pengerjaan ujian untuk siswa ini? Siswa akan mengulang ujian dari awal dan jawaban sebelumnya akan dibersihkan.')">
                                    @csrf
