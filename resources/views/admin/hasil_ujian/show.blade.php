...
<main class="ml-0 md:ml-72 p-4 md:p-10 min-h-screen">
    <a href="{{ route('admin.hasil_ujian.index') }}" class="inline-flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors font-bold text-sm mb-6">
    ...
    <div class="bg-indigo-600 rounded-[2.5rem] shadow-xl p-8 md:p-10 text-white mb-10 relative overflow-hidden">
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-4 md:px-8 py-3 border border-slate-300 text-center no-print">
                            <form action="{{ route('admin.hasil_ujian.reset', $h->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin membuka akses ujian kembali untuk siswa ini? Nilai saat ini TIDAK akan dihapus sampai siswa mengumpulkan jawaban yang baru.')">
                                @csrf
                                @method('DELETE')
