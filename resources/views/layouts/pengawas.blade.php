...
<head>
    ...
    <title>@yield('title', 'Panel Guru/Pengawas') | {{ $settings->app_name ?? 'CBT Aplikasi' }}</title>
...
<body class="text-slate-800">
    ...
    <aside id="main-sidebar" class="fixed left-0 top-0 h-screen w-72 bg-white border-r border-slate-200 z-50 flex flex-col transition-all duration-300 -translate-x-full md:translate-x-0">
        <div class="p-8 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="h-10 w-10 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-200 overflow-hidden">
                    @if(isset($settings) && $settings->logo_url)
                ...
                <div>
                    <h1 class="font-bold text-base text-slate-800 leading-tight">Panel Guru</h1>
                    <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $settings->app_name ?? 'CBT' }}</span>
                </div>
            </div>
        ...
        <nav id="sidebar-nav" class="flex-1 px-4 space-y-2 overflow-y-auto mt-4">
            ...
            <a href="{{ route('pengawas.dashboard') }}" 
               class="sidebar-link flex items-center gap-3 px-4 py-3.5 rounded-2xl font-semibold text-slate-600 transition-all {{ request()->routeIs('pengawas.dashboard') ? 'active' : '' }}">
               <i data-lucide="layout-dashboard" class="w-5 h-5"></i>
               <span>Pemantauan Ujian</span>
            </a>

            <a href="{{ route('pengawas.bank_soal.index') }}" 
               class="sidebar-link flex items-center gap-3 px-4 py-3.5 rounded-2xl font-semibold text-slate-600 transition-all {{ request()->routeIs('pengawas.bank_soal.*') ? 'active' : '' }}">
               <i data-lucide="database" class="w-5 h-5"></i>
               <span>Bank Soal</span>
            </a>

            <a href="{{ route('pengawas.hasil_ujian.index') }}" 
               class="sidebar-link flex items-center gap-3 px-4 py-3.5 rounded-2xl font-semibold text-slate-600 transition-all {{ request()->routeIs('pengawas.hasil_ujian.*') ? 'active' : '' }}">
               <i data-lucide="file-check-2" class="w-5 h-5"></i>
               <span>Hasil Ujian CBT</span>
            </a>

            <a href="{{ route('pengawas.siswa_aktif.index') }}" 
               class="sidebar-link flex items-center gap-3 px-4 py-3.5 rounded-2xl font-semibold text-slate-600 transition-all {{ request()->routeIs('pengawas.siswa_aktif.*') ? 'active' : '' }}">
        ...
        <div class="p-6 border-t border-slate-100 bg-slate-50/50 md:hidden">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div class="overflow-hidden">
                    <p class="text-sm font-bold text-slate-800 truncate">{{ auth()->user()->nama ?? 'Guru' }}</p>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Guru / Pengawas</p>
                </div>
            </div>
    ...
    <header class="hidden md:flex items-center justify-between bg-white border-b border-slate-200 py-3.5 px-8 ml-0 md:ml-72 sticky top-0 z-40 shadow-sm">
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-2.5">
                ...
                <div class="text-left">
                    <p class="text-xs font-bold text-slate-800 leading-none">{{ auth()->user()->nama ?? 'Guru' }}</p>
                    <p class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">Guru / Pengawas</p>
                </div>
            </div>
